...
const CompoundPage = () => {
  const [ compoundForm, setCompoundForm ] = useState<Compound>({ amountInvested: 1000, apyRate: 10, period: 365, duration: 1 })
  ...
  const calculate = (amountInvested: number, apyRate: number, period: number, duration: number) => {
    ...
    return amountInvested * Math.pow(1 + (apyRate/100/period), (period * duration));
  }
